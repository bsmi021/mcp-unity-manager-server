...
import { TOOL_NAME, TOOL_DESCRIPTION, TOOL_PARAMS, ManageSceneParams } from "./manageSceneParams.js"; // Import TOOL_PARAMS
import { UnityBridgeClient } from "../services/UnityBridgeClient.js";
import { logger } from "../utils/logger.js";

/**
 * Registers the manage_scene tool with the MCP server.
 * This tool interacts with the Unity Editor via the UnityBridgeClient to manage scenes.
 *
 * @param server - The McpServer instance.
 * @param unityBridgeClient - The initialized UnityBridgeClient instance.
 */
export const manageSceneTool = (server: McpServer, unityBridgeClient: UnityBridgeClient): void => {

    /**
     * Processes the manage_scene tool request.
     ...
     */
    const processRequest = async (args: ManageSceneParams) => {
        logger.info(`[${TOOL_NAME}] Received request`, { action: args.action, scene_path: args.scene_path, save_mode: args.save_mode });
